...
- **[#5](https://gitlab.huaun.com/rd.huaun/h-codeflow-framework/-/issues/5)** FE/QA 自检清单新增 B-4 字段映射表逐列核对，防止前后端字段不一致
- **[#7](https://gitlab.huaun.com/rd.huaun/h-codeflow-framework/-/issues/7)** 新增铁律"残留代码不构成 Spec"，阻止 Intake 将已有代码误判为新需求
- **[#8](https://gitlab.huaun.com/rd.huaun/h-codeflow-framework/-/issues/8)** 统一 Spec 目录命名格式为 `YYYY-MM-DD_hh-mm_<name>`，消除 PM Agent 创建目录时的格式歧义
- **[#9](https://gitlab.huaun.com/rd.huaun/h-codeflow-framework/-/issues/9)** 主会话 Intake 三问前主动加载 domain-ontology Skill，确保业务术语准确
- **[#10](https://gitlab.huaun.com/rd.huaun/h-codeflow-framework/-/issues/10)** domain-ontology 新增术语铁律，防止 Agent 自行发明业务术语
- **[#11](https://gitlab.huaun.com/rd.huaun/h-codeflow-framework/-/issues/11)** Workflow C 并行模式强制主会话为每个 Agent 创建独立 Worktree，禁止共用分支
- **[#12](https://gitlab.huaun.com/rd.huaun/h-codeflow-framework/-/issues/12)** Arch Agent Research 阶段强制加载后端代码模板，确保设计与实现结构一致
