...
```bash
# 修复
chmod +x ../h-codeflow-framework/templates/init-project.sh
```
...
```bash
# 确认目录结构
ls ../h-codeflow-framework/templates/init-project.sh
...
```bash
bash ../h-codeflow-framework/templates/init-subproject.sh ./my-subproject fe "My Project"
```
...
```bash
bash ../h-codeflow-framework/templates/init-project.sh . "My Project Name"
```
...
**原因**：文件中没有 `h-codeflow-framework:core` 标记行。

**排查步骤**：

1. 打开被警告的文件，搜索 `h-codeflow-framework`
